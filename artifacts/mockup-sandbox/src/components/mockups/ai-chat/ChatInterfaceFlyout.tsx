...
import {
  ...
  Share, Sparkles, Store, ChevronLeft,
  Code, PenTool, Globe, FileText,
  MoreHorizontal, Clock, TrendingUp,
} from 'lucide-react';

/* ─── Design 1 확장 — "새 대화 시작" 토글 날개 ──────────────────────
   Design 1(ChatInterface)와 동일한 사이드바 + 가운데 영역.
   유일한 추가: '새 대화' 버튼을 누르면 사이드바 오른쪽으로
   날개처럼 펼쳐지는 토글 패널.
...
/* ── Flyout data ── */
const RECENTLY_USED = [
  { label: '회의록 문장정리', desc: '3분 전 사용', grad: 'from-blue-500 to-indigo-500', emoji: '📝' },
  { label: '이메일 문체변경', desc: '오늘 오전 사용', grad: 'from-violet-500 to-purple-500', emoji: '✉️' },
  { label: '코드 리뷰 비서', desc: '어제 사용', grad: 'from-indigo-500 to-blue-600', emoji: '💻' },
];
...
const MOST_POPULAR = [
  { label: '코드 리뷰 비서', desc: '18.2k명 사용 중', grad: 'from-indigo-500 to-blue-600', emoji: '💻' },
  { label: '번역 비서', desc: '15.7k명 사용 중', grad: 'from-emerald-500 to-teal-400', emoji: '🌐' },
  { label: '데이터 분석 비서', desc: '11.9k명 사용 중', grad: 'from-cyan-500 to-sky-500', emoji: '📊' },
];
...
function FlyoutList({
  ...
  icon: typeof Clock;
  title: string;
  items: { label: string; desc: string; grad: string; emoji: string }[];
}) {
  return (
    <div className="mb-5 last:mb-0">
      <div className="flex items-center gap-1.5 mb-2 px-1">
        <Icon className="w-3.5 h-3.5 text-[#A8A6C0]" />
        <h4 className="text-[11px] font-bold text-[#6B6882] uppercase tracking-wider">{title}</h4>
      </div>
      <div className="flex flex-col gap-1">
        {items.map((item) => (
          <button
            key={item.label}
            type="button"
            className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-left hover:bg-[#F0EEFF] transition-colors group"
          >
            <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${item.grad} flex items-center justify-center text-sm shadow-sm shrink-0`}>
              {item.emoji}
            </div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-[#1A1826] leading-snug truncate group-hover:text-[#4F46E5] transition-colors">
                {item.label}
              </p>
              <p className="text-[10px] text-[#A8A6C0] leading-snug truncate">{item.desc}</p>
            </div>
          </button>
        ))}
      </div>
...
function FlyoutPanel({ onClose }: { onClose: () => void }) {
  return (
    <div className="absolute top-0 left-[280px] h-full w-[280px] bg-white border-r border-[#E4E2F0] shadow-[8px_0_24px_-8px_rgba(79,70,229,0.18)] z-10 flex flex-col">
      <div className="h-12 flex items-center justify-between px-4 border-b border-[#E4E2F0] shrink-0">
        <span className="text-xs font-bold text-[#1A1826] uppercase tracking-wider">빠른 시작</span>
        <button type="button" onClick={onClose} aria-label="패널 닫기" className="text-[#A8A6C0] hover:text-[#1A1826] transition-colors p-1 rounded-md hover:bg-[#F0EEFF]">
      ...
      <div className="p-3 shrink-0">
        <button
          type="button"
          className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl bg-[#4F46E5] hover:bg-[#4338CA] text-white transition-colors text-xs font-semibold shadow-sm shadow-[#4F46E5]/25"
        >
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        <FlyoutList icon={Clock} title="최근 내가 사용한 비서" items={RECENTLY_USED} />
        <FlyoutList icon={Sparkles} title="최근 등록된 비서" items={RECENTLY_REGISTERED} />
        <FlyoutList icon={TrendingUp} title="가장 인기 있는 비서" items={MOST_POPULAR} />
      </div>
    </div>
  );
}

const Sidebar = ({ flyoutOpen, onToggleFlyout }: { flyoutOpen: boolean; onToggleFlyout: () => void }) => {
  return (
    <div className="w-[280px] flex-shrink-0 bg-[#F9F8FF] flex flex-col h-full border-r border-[#E4E2F0] relative z-20">
      ...
      <div className="p-4 flex flex-col gap-4 flex-1 overflow-hidden">
        <button
          ...
          className={`w-full flex items-center justify-start gap-3 px-4 py-3 rounded-xl border transition-all duration-300 text-sm font-medium group shadow-sm ${
            flyoutOpen
              ? 'bg-[#E8E6FF] border-[#4F46E5]/40 text-[#1A1826]'
              : 'bg-[#F0EEFF] hover:bg-[#E8E6FF] text-[#1A1826] border-[#E4E2F0] hover:border-[#4F46E5]/40'
          }`}
        >
...
export function ChatInterfaceFlyout() {
  const [flyoutOpen, setFlyoutOpen] = useState(true);
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#F9F8FF] font-['Jeju_Samdasoo',sans-serif] relative">
      <Sidebar flyoutOpen={flyoutOpen} onToggleFlyout={() => setFlyoutOpen((v) => !v)} />
      {flyoutOpen && <FlyoutPanel onClose={() => setFlyoutOpen(false)} />}
      <MainPanel />
    </div>
  );
}
