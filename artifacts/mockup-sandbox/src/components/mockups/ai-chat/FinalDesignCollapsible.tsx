...
import {
  Plus, Search, Settings, ArrowUp, ChevronDown, ChevronLeft, ChevronRight,
...
/* ─── 최종 선택안 — 사이드바 접기/펼치기 ─────────────────────────
   FinalDesign과 동일한 디자인 컨셉.
   사이드바 헤더 우측에 토글 버튼 추가:
   · 펼침(기본): 240px 전체 메뉴
   · 접힘: 56px 아이콘 레일만 노출
   전환 시 부드러운 width 트랜지션 적용.
─────────────────────────────────────────────────────────────── */
  ...
  return (
    <div className="relative group/tip">
      ...
      <div className="pointer-events-none absolute left-full top-1/2 -translate-y-1/2 ml-2 z-50
        ...
        <div className="bg-[#1A1826] text-white text-[11px] font-medium px-2 py-1 rounded-md whitespace-nowrap shadow-lg">
          {label}
        </div>
        <div className="absolute top-1/2 -translate-y-1/2 -left-1 w-2 h-2 bg-[#1A1826] rotate-45" />
      </div>
    </div>
  );
}
...
function Sidebar({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  return (
    <aside
      className="relative bg-[#F9F8FF] border-r border-[#E4E2F0] flex flex-col h-full shrink-0 overflow-hidden transition-[width] duration-300 ease-in-out"
      style={{ width: open ? 240 : 56 }}
    >
      {/* Logo row + toggle button */}
      <div className="h-12 flex items-center border-b border-[#E4E2F0] shrink-0 px-3 relative">
        {/* Logo mark — always visible */}
        <div className="w-7 h-7 rounded-lg bg-[#4F46E5] flex items-center justify-center shadow-sm shadow-[#4F46E5]/30 shrink-0">
        ...
        {/* Brand name — visible only when open */}
        <span
          className={`ml-2 font-bold text-[#1A1826] text-sm tracking-tight whitespace-nowrap transition-[opacity,transform] duration-200 ${
        ...
        <button
          ...
          className={`absolute right-2 top-1/2 -translate-y-1/2
                      ...
                      transition-colors duration-150 ${!open ? 'right-2' : ''}`}
        >
          {open
      ...
      >
              ...
              {group.items.map((item, idx) => (
                <button key={idx} type="button"
                  className={`w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-left transition-all group ${
                    item.active ? 'bg-[#EEEEFF] border border-[#C7C3F7]' : 'hover:bg-[#F4F3FC] border border-transparent'
                  }`}>
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-semibold shrink-0 ${
                    item.active ? 'bg-[#4F46E5] text-white' : 'bg-[#F0EEFA] text-[#6B6882] group-hover:bg-[#E4E2F5]'
                  }`}>{item.initials}</span>
                  <span className={`text-xs leading-snug truncate ${item.active ? 'text-[#4F46E5] font-semibold' : 'text-[#6B6882]'}`}>
                    {item.title}
                  </span>
                </button>
              ))}
        ...
        <div className="border-t border-[#E4E2F0] px-3 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-[#4F46E5] flex items-center justify-center text-[11px] font-bold text-white shadow-sm shrink-0">
              김
            </div>
      ...
      {/* ── COLLAPSED icon rail ────────────────────────────── */}
      <div
        className={`absolute inset-0 top-12 flex flex-col items-center pt-3 gap-1 transition-[opacity] duration-200 ${
          open ? 'opacity-0 pointer-events-none' : 'opacity-100 delay-100'
        }`}
      >
        <Tooltip label="새 대화 시작">
          <button type="button"
        ...
        <Tooltip label="비서마켓">
          <button type="button"
            className="w-9 h-9 flex items-center justify-center rounded-lg text-[#A8A6C0] hover:bg-[#EEEEFF] hover:text-[#4F46E5] transition-colors">
        ...
        {/* Divider */}
        <div className="w-6 h-px bg-[#E4E2F0] my-1" />

        {/* Recent initials */}
        {HISTORY_GROUPS[0].items.map((item) => (
          <Tooltip key={item.initials} label={item.title}>
            <button type="button"
              className={`w-9 h-9 flex items-center justify-center rounded-full text-[11px] font-semibold transition-all ${
                item.active ? 'bg-[#4F46E5] text-white shadow-sm shadow-[#4F46E5]/30' : 'bg-[#F0EEFA] text-[#6B6882] hover:bg-[#E4E2F5]'
              }`}>
              {item.initials}
            </button>
          </Tooltip>
        ...
        <Tooltip label="설정">
          <button type="button"
            className="w-9 h-9 mb-3 flex items-center justify-center rounded-lg text-[#A8A6C0] hover:bg-[#F4F3FC] hover:text-[#6B6882] transition-colors">
            <Settings className="w-4 h-4" strokeWidth={1.8} />
          </button>
        </Tooltip>
      </div>
...
function ShortcutCard({ label, desc, icon: Icon }: { label: string; desc: string; icon: LucideIcon }) {
  return (
    <button type="button"
      className="group/card flex flex-col items-center gap-2 px-3 pt-4 pb-3 bg-white rounded-xl border border-[#E4E2F0]
      ...
      <div className="w-10 h-10 rounded-xl bg-[#F4F3FC] group-hover/card:bg-[#EEF0FF] flex items-center justify-center transition-colors duration-200">
        <Icon className="w-5 h-5 text-[#A8A6C0] group-hover/card:text-[#4F46E5] transition-colors duration-200"
          strokeWidth={1.8} />
      </div>
      <span className="text-xs font-semibold text-[#1A1826] leading-snug whitespace-nowrap">{label}</span>
      <span className="block text-[10.5px] text-[#A8A6C0] leading-snug
      ...
      <div className="h-12 flex items-center justify-end px-6 bg-white border-b border-[#E4E2F0] shrink-0">
        <div className="flex items-center gap-2">
          ...
          <button type="button" aria-label="공유"
            className="text-[#A8A6C0] hover:text-[#6B6882] transition-colors p-2 rounded-lg hover:bg-[#F4F3FC]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-10 pb-6 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        ...
        <div className="text-center mb-7">
          <h1 className="font-bold tracking-tight leading-[1.08]">
            ...
            <span className="block text-[52px]" style={{
        ...
        {/* Mode tabs */}
        <div role="tablist" aria-label="대화 모드"
          className="flex gap-1 p-1 rounded-xl bg-[#F4F3FC] border border-[#E4E2F0] mb-6">
          {MODES.map((m, i) => (
            <button key={m} role="tab" type="button" aria-selected={mode === i} onClick={() => setMode(i)}
...
export function FinalDesignCollapsible() {
