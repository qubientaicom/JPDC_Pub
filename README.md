...
- 대화 입력창 상단에 현재 선택된 비서명과 첨부파일을 칩으로 표시하고 긴 이름은 말줄임 처리
- 활성 비서가 있으면 `비서 다시 선택` 버튼을 제공하고 목록에서 현재 비서를 체크 표시하도록 개선
- 다른 비서를 선택해도 기존 대화를 즉시 전환하지 않고, 질문을 전송하는 순간 기존 대화를 저장한 뒤 선택한 비서와 새 대화를 시작하도록 변경
- 비서 전환 대화의 첫 영역에 비서별 소개와 대화 스타터를 표시하고, 기존 대화 저장 및 새 대화 시작 안내 추가
- 다른 비서를 선택한 전송 대기 상태에서는 질의 입력창에 기존 대화 저장과 새 비서 대화 시작 안내를 표시
- 비서 목록 전달 과정의 `assistantOrder is not defined` 런타임 오류 수정

### 2026-09-10

- 비서 생성·수정 화면을 좌측 설정 폼과 우측 테스트 패널로 재구성하고 저장 후 현재 초안을 시험할 수 있는 테스트 대화 흐름 추가
- 설명, 지침, 금지사항, 대화 스타터, 첫 메시지에 비서 이름·카테고리·설명을 반영하는 항목별 `자동생성` 기능 추가
- 저장 완료 후 관리자 `승인요청` 상태를 표시하고 중복 요청을 방지하도록 승인 요청 흐름 개선
- `비공개`·`지정자`는 승인 없이 바로 사용하고, `부서`·`본부`·`전사`는 관리자 승인을 필수로 요청하도록 공개 범위 정책 분기
- 금지사항과 대화 스타터의 자동생성 버튼 및 생성 문장 사이 여백을 조정해 가독성 개선
...
- API 요청에는 세션 쿠키를 사용하며, 프론트엔드는 `credentials: include` 방식으로 연동합니다.# JPDC AI
